...
var conn = new WebSocket(`ws://${hostname.split(':')[0]}:8090/?token={{ $userchat->token }}`);

var from_user_id = "{{ $userchat->id }}";
...
conn.onmessage = function(e){
	...
	if(data.image_link)
	{
		document.getElementById('message_area').innerHTML = `<img src="/uploads/all/${data.image_link}" class="img-thumbnail img-fluid" />`;
	}
			...
			for(var count = 0; count < data.data.length; count++)
			{
				...
				<li class="list-group-item">
					<div class="row">
						...
						<div class="col col-3">
							<button type="button" name="send_request" class="btn btn-primary btn-sm float-end" onclick="send_request(this, `+from_user_id+`, `+data.data[count].id+`)"><i class="bi bi-envelope-paper"></i></button>
						</div>
					</div>
				</li>
				`;
	...
	if(data.response_load_notification)
	{
		...
		for(var count = 0; count < data.data.length; count++)
		{
			...
			else
			{
				if(data.data[count].status == 'Pending')
				{
					html += '<button type="button" class="btn btn-danger btn-sm float-end" onclick="process_chat_request('+data.data[count].id+', '+data.data[count].from_user_id+', '+data.data[count].to_user_id+', `Reject`)">X</button>&nbsp;';
					html += '<button type="button" class="btn btn-success btn-sm float-end" onclick="process_chat_request('+data.data[count].id+', '+data.data[count].from_user_id+', '+data.data[count].to_user_id+', `Approve`)"><i class="bi bi-check"></i></button>';
				}
				else
				{
					html += '<button type="button" name="send_request" class="btn btn-danger btn-sm float-end">Request Rejected</button>';
	...
	if(data.response_connected_chat_user)
	{
		...
		if(data.data.length > 0)
		{
			for(var count = 0; count < data.data.length; count++)
			{
				...
				{
					html += '<span class="text-success online_status_icon" id="status_'+data.data[count].id+'"><i class="bi bi-circle"></i></span>';

					last_seen = 'Online';
				}
				else
				{
					html += '<span class="text-danger online_status_icon" id="status_'+data.data[count].id+'"><i class="bi bi-circle"></i></span>';
	...
	if(data.message)
	{
		...
		if(data.from_user_id == from_user_id)
		{
			...
			{
				icon_style = '<span class="text-primary float-end" id="chat_status_'+data.chat_message_id+'"><i class="bi bi-check-all"></i></span>';
			}
	...
	if(data.chat_history)
	{
		...
		for(var count = 0; count < data.chat_history.length; count++)
		{
			if(data.chat_history[count].from_user_id == from_user_id)
			{
				...
				{
					icon_style = '<span id="chat_status_'+data.chat_history[count].id+'" class="float-end"><i class="bi bi-check text-muted"></i></span>';
				}

				if(data.chat_history[count].message_status == 'Send')
				{
					icon_style = '<span id="chat_status_'+data.chat_history[count].id+'" class="float-end"><i class="bi bi-check-all text-muted"></i></span>';
				}

				if(data.chat_history[count].message_status == 'Read')
				{
					icon_style = '<span class="text-primary float-end" id="chat_status_'+data.chat_history[count].id+'"><i class="bi bi-check-all"></i></span>';
				}
	...
	if(data.update_message_status)
	{
		...
		if(chat_status_element)
		{
			...
			{
				chat_status_element.innerHTML = '<i class="bi bi-check-all text-primary"></i>';
			}
			if(data.update_message_status == 'Send')
			{
				chat_status_element.innerHTML = '<i class="bi bi-check-all text-muted"></i>';
			}
		}
...
function make_chat_area(user_id, to_user_name)
{
	...
	document.getElementById('close_chat_area').innerHTML = '<button type="button" id="close_chat" class="btn btn-danger btn-sm float-end" onclick="close_chat();">X</button>';
