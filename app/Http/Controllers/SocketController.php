...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['token'])) {
            UserChat::where('token', $queryarray['token'])->update(['connection_id' => $conn->resourceId, 'user_status' => 'Online']);

            $user_id = UserChat::select('id')->where('token', $queryarray['token'])->get();
    ...
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        if (preg_match('~[^\x20-\x7E\t\r\n]~', $msg) > 0) {
            ...
            file_put_contents(public_path('/uploads/all/') . $image_name, $msg);
        ...
        if (isset($data->type)) {
            ...
            if ($data->type == 'request_load_unconnected_user') {
                echo "from " . $data->from_user_id . "\r\n";
                $user_data = UserChat::select('id', 'name', 'user_status', 'user_image')
            ...
            if ($data->type == 'request_load_unread_notification') {
                ...
                foreach ($notification_data as $row) {
                    ...
                    $user_data = UserChat::select('name', 'user_image')->where('id', $user_id)->first();
                ...
                $sender_connection_id = UserChat::select('connection_id')->where('id', $data->user_id)->get();
            ...
            if ($data->type == 'request_connected_chat_user') {
                ...
                foreach ($user_id_data as $user_id_row) {
                    ...
                    $user_data = UserChat::select('id', 'name', 'user_image', 'user_status', 'updated_at')->where('id', $user_id)->first();
            ...
            if ($data->type == 'request_chat_history') {
                ...
                $receiver_connection_id = UserChat::select('connection_id')->where('id', $data->from_user_id)->get();
            ...
            if ($data->type == 'check_unread_message') {
                ...
                $sender_connection_id = UserChat::select('connection_id')->where('id', $data->from_user_id)->get(); //send number of unread message

                $receiver_connection_id = UserChat::select('connection_id')->where('id', $data->to_user_id)->get(); //send message read status
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['token'])) {
            UserChat::where('token', $queryarray['token'])->update(['connection_id' => 0, 'user_status' => 'Offline']);

            $user_id = UserChat::select('id', 'updated_at')->where('token', $queryarray['token'])->get();
